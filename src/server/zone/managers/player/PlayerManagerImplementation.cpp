		...
		if (attacker->isPet()) {
			...
			for (int j = 0; j < entry->size(); ++j) {
				...
				if (xpType != "jedi_general")
					...
					xpAmount *= 0.09f;   //xpAmount *= 0.2f; // x1 Experiencia
...
bool PlayerManagerImplementation::increaseOnlineCharCountIfPossible(ZoneClientSession* client) {
  ...
  ////// SI SE QUIERE QUE LOS ADMIN TAMBI'EN ENTREN EN QUE SOLO PUEDEN TENER UN NUMERO DE PERSONAJES ONLINE A LA VEZ ////
	//		if (ghost != NULL && ghost->getAdminLevel() > 0)
	//			continue;
	//		else if (player->getClient() == session)
	////////////////////////////////////
		 if (player->getClient() == session)  //// QUITAR ESTA LINEA SI SE QUIERE LO ANTERIOR
		 //////////////////////////
				++onlineCount;
		}
	}
