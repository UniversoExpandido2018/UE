...
bool PlayerCreationManager::createCharacter(ClientCreateCharacterCallback* callback) const {
	...
	if (client->getCharacterCount(zoneServer.get()->getGalaxyID()) >= 4) {
		ErrorMessage* errMsg = new ErrorMessage("Create Error", "Estas limitado a 4 personajes por Galaxia", 0x0);
	...
	if (ghost != NULL) {
		...
		if (!freeGodMode) {
			...
			//			if (timeVal.miliDifference() < 3600000) {  // 1 hora
							if (timeVal.miliDifference() < 360000) {  // 10 minutos
								ErrorMessage* errMsg = new ErrorMessage("Create Error", "Solo se permite crear un personaje cada 10 minutos. Repetir el intento antes de que pase el tiempo reseteara el contador.", 0x0);
								client->sendMessage(errMsg);
				...
				//  if (lastCreatedTime.miliDifference() < 3600000) {  // 1 hora
						if (lastCreatedTime.miliDifference() < 360000) {  // 10 munutos
							ErrorMessage* errMsg = new ErrorMessage("Create Error", "Solo se permite crear un personaje cada 10 minutos. Repetir el intento antes de que pase el tiempo reseteara el contador.", 0x0);
							client->sendMessage(errMsg);
	...
	box->setPromptText("Solo se permite crear un personaje cada 10 minutos. Intentar crear otro personaje o borrarlo antes de que pase el tiempo reseteara el contador.");
