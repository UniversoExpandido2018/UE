	...
	if (object->isTangibleObject() && containmentType == 4) {
		...
		if (tanoData != NULL) {
			...
			if (!races->contains(race.hashCode())) {
				errorDescription = "No cumples los requisitos para llevar este objeto";
		...
		if (object->isArmorObject()) {
			...
			if (!playerManager->checkEncumbrancies(creo, cast<ArmorObject*>(object))) {
				errorDescription = "Careces de las Stats secundarias necesarias para equiparte con este objeto";
