...
Account* AccountManager::validateAccountCredentials(LoginClient* client, const String& username, const String& password) {
	...
	if(account == NULL) {
		...
		if (isAutoRegistrationEnabled() && client != NULL) {
			...
			if(client != NULL)
				client->sendErrorMessage("Login Error", "El Registro Automatico esta desactivado. Por favor, contacta con un Administrador para conseguir acceso.");
			return NULL;
		}
	}
	...
	if(!account->isActive()) {
		...
		if(client != NULL)
			client->sendErrorMessage("Cuenta Desactivada", "Contacta con un Administrador para saber que ha sucedido.");
