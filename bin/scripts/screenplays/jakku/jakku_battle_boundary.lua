...
function pvp:notifySpawnArea(pActiveArea, pMovingObject)
  ...
  --    player:playMusicMessage("sound/music_space_battle_lp_01.snd")
      player:sendSystemMessage("Has entrado en la Batalla de Jakku!")
      player:setFactionStatus(2)
    end


    if (player:isNeutral()) then
      player:sendSystemMessage("Debes ser miembro de una faccion para unirte a la Batalla de Jakku!")
...
--      player:sendSystemMessage("You have deserted in the heat of battle. For this you will be sent to the Cratertown Arena.")
	--	  player:playMusicMessage("sound/music_themequest_fail_imperial.snd")
      player:sendSystemMessage("Has desertado del calor de la batalla. Eres un cobarde!")
  --    player:teleport(4444, 7, -5168, 0)
      else
    --  	player:playMusicMessage("sound/music_space_battle_01_victory.snd")
        player:sendSystemMessage("Estas dejando la zona de la batalla.")
    --    player:teleport(4331, 9.1, -5130, 0)
      end
    return 0
  end)
